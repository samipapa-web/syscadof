...
try:
    ...
    if nom_clean.lower().endswith(".xlsx"):
        df = pd.read_excel(chemin_clean, header=2)
        df.reset_index(drop=True, inplace=True)

        # Suppression colonnes 14 à 16
        if len(df.columns) > 15:
            df.drop(columns=df.columns[13:16], inplace=True)
        ...
        print(f"Format non supporté : {nom_clean}", file=sys.stderr)
        sys.exit(1)
